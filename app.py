...
.login-box {
    ...
    max-width: 400px;
    margin: 4rem auto 0;
    box-shadow: 0 8px 40px rgba(180,80,100,0.1);
    text-align: center;
}
...
# ── Auth Screen ───────────────────────────────────────────────────────────────
def auth_page():
    st.markdown('<div class="main-title">Nossa Agenda</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">o calendário de vocês dois 💕</div>', unsafe_allow_html=True)
...
def main_app():
    ...
    with col1:
        st.markdown(f'<div class="main-title">Nossa Agenda 💕</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="sub-title">Olá, {nome}!</div>', unsafe_allow_html=True)
    with col2:
        st.write("")
        st.write("")
    ...
    with st.expander("➕ Adicionar novo evento", expanded=False):
        with st.form("novo_evento"):
            ...
            descricao = st.text_area("Descrição / observações", height=80)
            ...
            if salvar:
                if not titulo:
                    ...
                    add_event(titulo, descricao, dt, CATEGORIAS[categoria_label], user.id)
    ...
    st.markdown('<div class="section-header">📅 Próximos eventos</div>', unsafe_allow_html=True)

    if not upcoming:
        st.info("Nenhum evento agendado. Que tal planejar algo juntos? 💕")
    else:
        for ev in upcoming:
            dt = datetime.fromisoformat(ev["data_hora"]).astimezone(BR_TZ)
            criador = ev.get("profiles", {})
            criador_nome = criador.get("nome", "?") if criador else "?"
            cat_emoji = next((k.split()[0] for k, v in CATEGORIAS.items() if v == ev.get("categoria")), "📅")
            ...
            with st.container():
                ...
                with c1:
                    ...
                    <div class="event-card">
                        <div class="event-title">{cat_emoji} {ev['titulo']}</div>
                ...
                with c2:
                    if ev.get("user_id") == user.id:
                        st.write("")
                        st.write("")
                        if st.button("🗑️", key=f"del_{ev['id']}", help="Deletar evento"):
                            delete_event(ev["id"])
                            st.rerun()

    if past:
        with st.expander(f"🕰️ Histórico ({len(past)} eventos)", expanded=False):
            for ev in past:
                dt = datetime.fromisoformat(ev["data_hora"]).astimezone(BR_TZ)
                criador = ev.get("profiles", {})
                criador_nome = criador.get("nome", "?") if criador else "?"
                ...
                <div class="event-card past">
                    <div class="event-title">{cat_emoji} {ev['titulo']}</div>
                    <div class="event-meta">📆 {dt.strftime('%d/%m/%Y às %H:%M')} &nbsp;|&nbsp; 👤 {criador_nome}</div>
                </div>
                """, unsafe_allow_html=True)
